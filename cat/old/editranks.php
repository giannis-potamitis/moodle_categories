...
$rankform = new local_cat_rank_form($url);
$rankformsubmit = $rankform->get_data();

// ranks form
$ranks = $DB->get_records('cat_ranks', array('rankid' => $rankid), 'priority ASC');
$ranks_custom = new stdClass();
$ranks_custom->ranks = $ranks;
$franks = new local_cat_ranks_form($url, $ranks_custom);

$franks_submit = $franks->get_data();

// add form
$addform = new local_cat_add_form($url);
$addsubmit = $addform->get_data();
 
$redefine = false; 
if ($rankformsubmit) { // rank form submit
	$rank->name = $rankformsubmit->rankname;
	$DB->update_record('cat_rank', $rank);

}	
else if ($franks_submit) {
	print_r($_POST);
	$fdata = $franks_submit;
	foreach ($ranks as $r) {
		$update = 'update' . $r->id;
    ...
    if (isset($fdata->$update)) { // update
    	$r->description = $fdata->$des;
    	$DB->update_record('cat_ranks', $r);
    	break;
    }
    else if (isset($fdata->$delete)) { // delete
    	...
    	$redefine = true;
    	break;
    } else if (isset($fdata->$up) || isset($fdata->$down)) { // move up or down
    	$swappriority = $r->priority + (isset($fdata->$up) ? -1 : 1);
    	$swap = $DB->get_record('cat_ranks', array('rankid' => $rankid, 'priority' => $swappriority), '*', MUST_EXIST);
    	$swap->priority += isset($fdata->$up) ? 1 : -1;
    	$DB->update_record('cat_ranks', $swap);
    	
    	$r->priority += isset($fdata->$up) ? -1 : 1;
    	$DB->update_record('cat_ranks', $r);
...
else if ($addsubmit) { // add form submit
	$DB->insert_record('cat_ranks', array('rankid' => $rankid, 'description' => $addsubmit->newrank, 'priority' => $rank->nextpriority));
	$rank->nextpriority++;
	$DB->update_record('cat_rank', $rank);
	$ranks = $DB->get_records('cat_ranks', array('rankid' => $rankid), 'priority ASC');
	$redefine = true;	
...
foreach	($ranks as $r) {
	...
	$sdata->$description = $r->description;
}	
$franks->set_data($sdata);

...
//===== 1.9.0 upgrade line ======//
    ...
    if ($oldversion < 2011121100) {

        // Changing type of field grade on table assignment_submissions to number
        $table = new xmldb_table('assignment_submissions');
        $field = new xmldb_field('grade', XMLDB_TYPE_NUMBER, '11, 5', null, XMLDB_NOTNULL, null, '0', 'data2');

        // Launch change of type for field grade
        $dbman->change_field_type($table, $field);

        // assignment savepoint reached
        upgrade_mod_savepoint(true, 2011121100, 'assignment');
    }
